...
export default function ArticleFeedback({ articleId }: { articleId: number }) {
  ...
  return (
    <div className="flex items-center gap-4 rounded-lg border border-border bg-card p-4">
      ...
      <button onClick={() => handle("helpful")} className={`p-2 rounded-lg ${feedback === "helpful" ? "bg-emerald-950 text-emerald-400" : "text-text-muted hover:text-text-primary"}`}><ThumbsUp size={18} /></button>
      <button onClick={() => handle("not")} className={`p-2 rounded-lg ${feedback === "not" ? "bg-red-950 text-red-400" : "text-text-muted hover:text-text-primary"}`}><ThumbsDown size={18} /></button>
    </div>
  );
}
