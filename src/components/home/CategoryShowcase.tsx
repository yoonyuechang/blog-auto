    ...
    <section className="mx-auto max-w-6xl px-4 py-16">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {CATEGORIES.map((cat, i) => {
          ...
          return (
            <AnimatedSection key={cat.slug} delay={i * 80} direction="up">
              <button
                onClick={() => router.push(`/category/${cat.slug}`)}
                className={`group w-full rounded-xl border bg-card p-5 text-left transition-all hover:shadow-lg hover:shadow-emerald-950/20 hover-lift ${cat.borderColor}`}
              >
